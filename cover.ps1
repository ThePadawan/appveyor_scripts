function cover
{
  ...
  process
  {
    ...
    $matchingProjects | Foreach-Object {
      ...
      .\OpenCover.*\tools\OpenCover.Console.exe -oldstyle -mergeoutput "-target:C:\Program Files\dotnet\dotnet.exe" "-targetargs:test $projectName" -returntargetcode "-filter:$coverageFilter" -hideskipped:all "-output:$coverageXmlPath" -log:All
    }
